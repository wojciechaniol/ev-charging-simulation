...
# Start the services
Write-Host "🚀 Starting CP services..." -ForegroundColor Yellow
docker compose -f $OverrideFile up -d

if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Failed to start CP services" -ForegroundColor Red
    exit 1
}

Write-Host ""
Write-Host "⏳ Waiting for services to start (5 seconds)..." -ForegroundColor Yellow
...
if ($engineRunning) {
    Write-Host "   ✅ CP Engine (ev-cp-e-$CpNumber) is running" -ForegroundColor Green
} else {
    Write-Host "   ❌ CP Engine failed to start" -ForegroundColor Red
}

$monitorRunning = docker ps --filter "name=ev-cp-m-$CpNumber" --format "{{.Names}}"
if ($monitorRunning) {
    Write-Host "   ✅ CP Monitor (ev-cp-m-$CpNumber) is running" -ForegroundColor Green
} else {
    Write-Host "   ❌ CP Monitor failed to start" -ForegroundColor Red
}
...
Write-Host ""
Write-Host "✅ $CpId deployment complete!" -ForegroundColor Green
